...
def BFS(startPoint, maxHeight, numOfSite, site, roadMap):
    queue = []
...
def UCS(startPoint, maxHeight, numOfSite, site, roadMap):
    pathLength = 0
    queue = []
    #Keep all the visited nodes in the list.
    visited = np.zeros([roadMap.shape[0], roadMap.shape[1]], dtype=np.int32)
...
if algorithm == "BFS":
    ...
    for site in siteList:
        BFS(startPoint, maxHeight, numOfSite, site, roadMap)
elif algorithm == "UCS":
    #print("in bfs")
    for site in siteList:
        UCS(startPoint, maxHeight, numOfSite, site, roadMap)
